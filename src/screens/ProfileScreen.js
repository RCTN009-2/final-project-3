    ...
    <View style={{ flex: 1, alignItems: 'center' }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 20, marginBottom: 8 }}>
        ...
        <View style={{ marginLeft: 16 }}>
          <Text style={{ fontWeight: 'bold', color: '#1E3A8A', fontSize: 18, marginBottom: 2 }}>
            {namaLengkap}
          </Text>
          <Text style={{ fontSize: 12, color: '#718096' }}>{profileData.email}</Text>
        </View>
      </View>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', width: '100%', padding: 16, borderWidth: 2, borderColor: '#718096', borderRadius: 8, marginBottom: 6 }}>
        <View style={{ alignItems: 'center', flexDirection: 'column' }}>
          <Text style={{ fontWeight: 'bold', marginBottom: 2 }}>Bookings</Text>
          <Text style={{ fontSize: 12, color: '#718096' }}>{bookData.length}</Text>
        </View>
        <View style={{ alignItems: 'center', flexDirection: 'column' }}>
        ...
        <View style={{ alignItems: 'center', flexDirection: 'column' }}>
          ...
          <Text style={{ fontSize: 12, color: '#718096' }}>{wishlist.length}</Text>
        </View>
      </View>

      <ScrollView>
        {bookData && bookData.map((item) => <CardSmall key={item.id} item={item} />)}
      </ScrollView>
    </View>
  );
};
